  ...
  public virtual void InitHand() {
    for (int f = 0; f < fingers.Length; ++f) {
      if (fingers[f] != null) {
		...
		/*
		if(LeapInputManager.instance!=null){
		LeapInputManager.instance.fingers=fingers;

			if(hand_.IsLeft){
				LeapInputManager.instance.leftHand=gameObject;
			}
			else if(hand_.IsRight){
				LeapInputManager.instance.rightHand=gameObject;
			}
		}
		*/
    UpdateHand ();

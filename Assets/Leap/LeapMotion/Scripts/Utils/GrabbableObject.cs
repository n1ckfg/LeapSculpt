...
public class GrabbableObject : MonoBehaviour {
  ...
  protected bool hovered_ = false;
	...
	public bool shatterSphereScene=false;
  ...
  public virtual void OnRelease() {
    ...
    if (breakableJoint != null) {
      ...
      if (breakJoint != null) {
		...
		if(shatterSphereScene){
			if(Vector3.Distance(transform.position,RebuildingSphere.instance.transform.position)<=RebuildingSphere.instance.hitRange){
				GetComponent<FragmentScript>().Reset();
				Destroy(GetComponent<GrabbableObject>());
					Destroy(GetComponent<Rigidbody>());
						
				//GetComponent<GrabbableObject>().enabled=false;
			//RebuildingSphere.instance.numBits++;
